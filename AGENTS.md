...
- Keep `Program.cs` wiring-only; middleware currently includes problem-details error handling, CORS, rate limiting, auth, and `/health`.
...
- Auth bootstrap depends on `provideAppInitializer(() => AuthService.checkAuth())` in `ClientApp/src/app/app.config.ts`.
