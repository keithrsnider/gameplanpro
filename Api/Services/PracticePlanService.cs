...
{
	Task<List<PracticePlanListResponse>> GetAllAsync(string userId);
	Task<PracticePlanDetailResponse> GetByKeyAsync(string userId, Guid key);
	Task<PracticePlanDetailResponse> CreateAsync(string userId, CreatePracticePlanRequest request);
	Task<PracticePlanDetailResponse> UpdateAsync(
		string userId, Guid key, UpdatePracticePlanRequest request
	);
	Task DeleteAsync(string userId, Guid key);
}

public class PracticePlanService(IPracticePlanRepository planRepo) : IPracticePlanService
{
	public async Task<List<PracticePlanListResponse>> GetAllAsync(string userId)
	{
		var plans = await planRepo.GetAllByUserAsync(userId);
		return plans.Select(pp => pp.ToListResponse()).ToList();
	}

	public async Task<PracticePlanDetailResponse> GetByKeyAsync(string userId, Guid key)
	{
	...
	public async Task<PracticePlanDetailResponse> CreateAsync(
		string userId, CreatePracticePlanRequest request)
	{
		var plan = request.ToEntity(userId);
